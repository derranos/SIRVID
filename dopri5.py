...
def dorpi_5(b, g, d, a, s, N1, t):
    ...
    # Параметры модели
    beta = b  # Коэффициент передачи 0.2
    gamma = g # Коэффициент выздоровления
    delta = d # Коэффициент смертности
    alpha = a  # Скорость вакцинации
    sigma = s  # Утрата иммунитета
    N = N1 # Общее население
    ...
    def dopri5(f, t_span, y0, tol=1e-6, h_min=1e-4, h_max=1.0):
        ...
        while t < t_span[1]:
            ...
            error = (sum(abs(y_new[j] - y_hat[j]) for j in range(len(y0))))**2  # Максимальная ошибка
